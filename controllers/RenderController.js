...
const RenderController = {
    ...
    getLatestRenders: async (limit = 1) => {
        let data, error, response;
        try {
            response = await RenderModel.find({}).sort({
                _id: -1
            }).limit(limit)
            if (response.length) {
                data = response
    ...
    getRenderByCountId: async (countField) => {
        let data, error, response;
        try {
            response = await RenderModel.findOne({
                count: Number(countField)
            })
            if (response) {
                data = response
            }
                ...
                response = await RenderModel.aggregate([{
					...
					},
                    {
                        $set: {
                            ...
                            //                             cond: {
                            //                                 $eq: ["$$likeItem.ip", ip]
                            //                             }, // Check if "ip" field matches the provided "ip" parameter
                            //                         },
                            //                     },
                            //                 },
                            //                 0,
